...
public class WoundInteractableObject : MonoBehaviour
{
    ...
    [SerializeField] private WoundDialogueManager dialogueManager;
    ...
    private void Awake()
    {
        ...
        if (dialogueManager == null)
            dialogueManager = WoundDialogueManager.Instance;
    ...
    private void OnSelectEntered(SelectEnterEventArgs args)
    {
        ...
        if (dialogueManager == null)
        {
            Debug.LogError($"[{objectName}] WoundDialogueManager is NULL (Instance not found or not initialized).");
            return;
        }
        ...
        if (!atStage)
        {
            ...
            // Prevent "pre-holding" from counting
            holdTimer = 0f;
            return;
        }
    ...
    private void CompleteCorrect()
    {
        ...
        if (advanceDialogueOnUse)
        {
            if (dialogueManager == null)
            {
                Debug.LogError($"[{objectName}] WoundDialogueManager is NULL — cannot advance!");
                return;
            }

            Debug.Log($"[{objectName}] Advancing dialogue now...");
    ...
    private void MaybeWrongFeedback()
    {
        ...
        if (Time.time - lastWrongTime >= wrongFeedbackCooldown)
        {
            ...
            if (dialogueManager != null)
            {
                ...
                Debug.Log(
                    $"{objectName} used at wrong stage (current " +
                    $"{WoundDialogueManager.NormalizeSegmentKey(dialogueManager.GetCurrentSegmentName())}:" +
                    $"{dialogueManager.GetCurrentLineIndex()})"
                );
            }
        }
